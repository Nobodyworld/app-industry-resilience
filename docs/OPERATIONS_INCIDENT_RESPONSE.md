...
2. Query `/observability/status`, `/observability/digest`, or the persisted catalogue at `/observability/snapshots` (or run `python scripts/observability_snapshot.py --store --list`) to review recent operation events, metric counts, registered health checks, and historical digests. Use `python scripts/observability_snapshot.py --compare <snapshot.json>` to diff against the most recent stored state. When live streaming is helpful, run `python scripts/observability_tail.py --follow` to watch events in real time without reopening the API.
