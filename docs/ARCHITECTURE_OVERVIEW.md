...
- **Connector Catalog:** `ConnectorExtension` plug-ins register `ConnectorRegistration` entries with the shared `ConnectorRegistry`, exposing identifiers, capabilities, owners, and health checks for each integration. The registry feeds `/meta/connectors`, Streamlit's configuration sidebar, observability digests, and the CLI (`make connectors-catalog`). Built-in entries cover the bundled sample CSV, BEA API, and Census ASM sources, and extensions can add new connectors without touching core services or UI code.
...
2. **Data acquisition** – `src.application.idiot_index_service.IdiotIndexService` requests data from adapters based on user selection (sample CSV, BEA, or Census ASM). Adapters rely on shared retry/caching utilities.
