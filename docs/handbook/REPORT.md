...
> Historical report: this document records a prior assessment snapshot. Current support policy is Python 3.13+ (see `README.md` and `pyproject.toml`).
...
- Historical note: at report time, GitHub Actions (`ci.yml`) ran `make check` on Python 3.9-3.11, uploaded coverage to Codecov, and executed a dedicated security job (gitleaks, pip-audit via `make security`, SBOM generation).
