...
The Idiot Index platform now exposes a first-class connector registry so integrations can publish metadata, capabilities, and health diagnostics without modifying core services.

## Surfaces

- **API:** `GET /meta/connectors` returns the catalog payload consumed by automation and observability dashboards.
- **CLI:** `make connectors-catalog` (or `python scripts/connectors_catalog.py --json --pretty`) prints the same dataset locally.
...
```bash
python scripts/scaffold_extension.py --name supply_chain --with-connector
```
