...
| GET | `/v1/context/momentum` | unchanged | optional `source_family`, `signal_type`, `series_id`, `start`, `end`, `limit` | `IndustryMomentumListResponse` | Canonical-only 38-series registry, family availability, snapshot summaries, and limits; no provider calls or alias. |
| GET | `/v1/context/momentum/{industry_code}` | unchanged | six-digit path code plus bounded momentum filters | `IndustryMomentumResponse` | Per-family available/partial/unmapped/empty/unavailable contract; every requested family unavailable is a sanitized 503. |
