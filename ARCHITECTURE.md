...
4. **Presentation** – Streamlit components render hero copy, tables, charts, and the deep-dive panel. Download buttons are powered by `prepare_download_artifacts`, which precomputes CSV/JSON/Excel payloads.
5. **Automation** – the agent toolkit wraps the same functions into dataclass-driven APIs, exposing JSON schemas for integrations.
