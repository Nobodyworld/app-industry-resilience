...
| Observability Sentinel | Verify `/health`, `/observability/status`, and CLI health probes; ensure recent events and metrics align with docs. When remote replication is enabled, confirm snapshots land in the configured S3 bucket and review stderr for replication warnings. | Post-deployment and when modifying instrumentation. | `python scripts/check_health.py`, `python scripts/observability_snapshot.py`, API smoke tests |
